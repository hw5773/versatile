/* packet-test.c
 * Routines for test dissection
 * Copyright 201x, Hyunwoo Lee <hwlee2014@mmlab.snu.ac.kr>
 *
 ...
 * data structures can go in the header file packet-test.h. If not, then
...
static int hf_test_field = -1;
static expert_field ei_test_EXPERTABBREV = EI_INIT;
...
static guint tcp_port_pref = test_TCP_PORT;
        ...
        void *data _U_)
{
    ...
    if (tvb_reported_length(tvb) < test_MIN_LENGTH)
        return 0;
     ...
     * is narrow and generally just contains the constant string 'test',
    ...
    /* Set the Protocol column to the constant string of test */
    ...
    col_set_str(pinfo->cinfo, COL_INFO, "test Request");
     ...
     * information. */
    expert_ti = proto_tree_add_item(test_tree, hf_test_field, tvb,
            offset, len, ENC_xxx);
    offset += len;
    /* Some fields or situations may require "expert" analysis that can be
     * specifically highlighted. */
    if ( TEST_EXPERT_condition )
        /* value of hf_test_field isn't what's expected */
        expert_add_info(pinfo, expert_ti, &ei_test_EXPERTABBREV);
...
proto_register_test(void)
{
    ...
    static ei_register_info ei[] = {
        { &ei_test_EXPERTABBREV,
          { "test.EXPERTABBREV", PI_SEVERITY, PI_GROUP,
            "EXPERTDESCR", EXPFILL }
        }
    };

    /* Register the protocol name and description */
    proto_test = proto_register_protocol("test",
            "TEST", "test");

    /* Required function calls to register the header fields and subtrees */
    proto_register_field_array(proto_test, hf, array_length(hf));
    proto_register_subtree_array(ett, array_length(ett));

    /* Required function calls to register expert items */
    expert_test = expert_register_protocol(proto_test);
    expert_register_field_array(expert_test, ei, array_length(ei));
     ...
     * proto_reg_handoff_test in the following.
     */
    test_module = prefs_register_protocol(proto_test,
            proto_reg_handoff_test);
     ...
     * will be accessible under Protocols->OSI->X.500-><TEST>
     * preferences node.
     */
    test_module = prefs_register_protocol_subtree(const char *subtree,
            proto_test, proto_reg_handoff_test);

    /* Register a simple example preference */
    prefs_register_bool_preference(test_module, "show_hex",
            "Display numbers in Hex",
            "Enable to display numerical values in hexadecimal.",
            &pref_hex);

    /* Register an example port preference */
    prefs_register_uint_preference(test_module, "tcp.port", "test TCP Port",
            " test TCP port if other than the default",
            10, &tcp_port_pref);
}
...
proto_reg_handoff_test(void)
{
    ...
    if (!initialized) {
         ...
         * dissect_test() returns the number of bytes it dissected (or 0
         * if it thinks the packet does not belong to test).
         */
        test_handle = create_dissector_handle(dissect_test,
                proto_test);
        initialized = TRUE;
         ...
         * the test_handle and the value the preference had at the time
         * you registered.  The test_handle value and the value of the
         * preference can be saved using local statics in this
         * function (proto_reg_handoff).
         */
        dissector_delete_uint("tcp.port", current_port, test_handle);
    }

    current_port = tcp_port_pref;

    dissector_add_uint("tcp.port", current_port, test_handle);
}

#if 0

/* Simpler form of proto_reg_handoff_test which can be used if there are
...
proto_reg_handoff_test(void)
{
    ...
    /* Use create_dissector_handle() to indicate that dissect_test()
     * returns the number of bytes it dissected (or 0 if it thinks the packet
     * does not belong to test).
     */
    test_handle = create_dissector_handle(dissect_test,
            proto_test);
    dissector_add_uint("tcp.port", test_TCP_PORT, test_handle);
}
#endif
